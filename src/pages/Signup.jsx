...
export const Signup = () => {
  ...
  return (
    <main className="min-h-screen bg-gradient-to-br from-black via-[#120506] to-[#25080b] px-4 py-10 text-white">
      <motion.section initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }} className="mx-auto w-full max-w-2xl rounded-3xl border border-[#009689]/35 bg-black/45 p-6 shadow-[0_0_80px_rgba(0,150,137,0.15)] backdrop-blur-xl sm:p-8">
        ...
        <form onSubmit={onSubmit} className="mt-6 grid gap-4 md:grid-cols-2">
          ...
          <label className="block text-sm">
            ...
            <select value={form.role} onChange={(event) => setForm((prev) => ({ ...prev, role: event.target.value }))} className="w-full rounded-xl border border-white/10 bg-black/40 px-3 py-2.5 text-white outline-none focus:border-[#009689]/70">
          ...
          <div className="md:col-span-2">
            <motion.button whileHover={{ scale: 1.01 }} whileTap={{ scale: 0.99 }} type="submit" className="inline-flex w-full items-center justify-center gap-2 rounded-xl bg-gradient-to-r from-[#009689] to-[#009689] px-4 py-2.5 font-semibold">
        ...
        <p className="mt-4 text-sm text-white/75">Already have an account? <Link to="/login" className="text-[#009689] hover:underline">Login</Link></p>
      </motion.section>
    </main>
  );
};
...
const Field = ({ label, name, type = "text", value, error, onChange }) => (
  <label className="block text-sm">
    ...
    <motion.input
      ...
      className="w-full rounded-xl border border-white/10 bg-black/40 px-3 py-2.5 text-white outline-none transition focus:border-[#009689]/70"
    />
    <AnimatePresence>{error ? <motion.span initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="mt-1 block text-xs text-teal-300">{error}</motion.span> : null}</AnimatePresence>
  </label>
);

const GoogleButton = ({ onClick }) => (
  <motion.button whileHover={{ scale: 1.01 }} whileTap={{ scale: 0.98 }} type="button" onClick={onClick} className="mt-3 flex w-full items-center justify-center gap-3 rounded-xl border border-white/10 bg-black/40 px-4 py-3 text-sm font-medium text-white transition hover:border-[#009689]/45">
...
const GoogleLogo = () => (
  <svg width="18" height="18" viewBox="0 0 48 48" aria-hidden="true">
    ...
    <path fill="#009689" d="M6.306 14.691l6.571 4.819C14.655 16.108 18.961 13 24 13c3.059 0 5.841 1.154 7.959 3.041l5.657-5.657C34.046 6.053 29.268 4 24 4c-7.682 0-14.303 4.337-17.694 10.691Z"/>
