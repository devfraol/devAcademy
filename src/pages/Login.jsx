...
export const Login = () => {
  ...
  return (
    <main className="min-h-screen bg-gradient-to-br from-black via-[#120506] to-[#25080b] px-4 py-10 text-white">
      <motion.section initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }} className="mx-auto w-full max-w-lg rounded-3xl border border-[#009689]/35 bg-black/45 p-6 shadow-[0_0_80px_rgba(0,150,137,0.15)] backdrop-blur-xl sm:p-8">
        ...
        <form onSubmit={onSubmit} className="mt-6 space-y-4">
          ...
          <label className="flex items-center gap-2 text-sm text-white/80">
            <input type="checkbox" checked={form.remember} onChange={(event) => setForm((prev) => ({ ...prev, remember: event.target.checked }))} className="h-4 w-4 rounded border-white/20 bg-black/40 accent-[#009689]" />
            Remember me
          </label>
        ...
        <p className="mt-4 text-sm text-white/75">No account yet? <Link to="/signup" className="text-[#009689] hover:underline">Create one</Link></p>
      </motion.section>
    </main>
  );
};
...
const GoogleButton = ({ onClick }) => (
  <motion.button whileHover={{ scale: 1.01 }} whileTap={{ scale: 0.98 }} type="button" onClick={onClick} className="mt-4 flex w-full items-center justify-center gap-3 rounded-xl border border-white/10 bg-black/40 px-4 py-3 text-sm font-medium text-white transition hover:border-[#009689]/45">
