...
export const Blog = () => {
  ...
  return (
    <section className="space-y-5 rounded-2xl border border-white/10 bg-zinc-900/65 p-5 backdrop-blur-xl">
      ...
      <form onSubmit={saveBlog} className="space-y-3 rounded-xl border border-white/10 bg-black/20 p-4"><h3 className="text-base font-semibold text-white">Add Blog Post</h3><input value={form.title} onChange={(event) => setForm((prev) => ({ ...prev, title: event.target.value }))} placeholder="Title" className="w-full rounded-xl border border-white/10 bg-zinc-900 px-3 py-2 text-zinc-100 outline-none focus:border-[#007a55]/60" /><input value={form.excerpt} onChange={(event) => setForm((prev) => ({ ...prev, excerpt: event.target.value }))} placeholder="Excerpt" className="w-full rounded-xl border border-white/10 bg-zinc-900 px-3 py-2 text-zinc-100 outline-none focus:border-[#007a55]/60" /><button type="submit" disabled={loading.submit} className="rounded-xl border border-[#007a55]/60 bg-[#007a55]/15 px-4 py-2 text-sm text-[#FF7C73]">{loading.submit ? "Saving..." : "Save Blog"}</button></form>
    </section>
  );
};
