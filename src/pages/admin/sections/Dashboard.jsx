    ...
    <section className="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-4">
      {statCards.map((card, index) => (
        <motion.article
          ...
          className="rounded-2xl border border-white/10 bg-zinc-900/70 p-5 backdrop-blur-xl transition hover:border-[#009689]/55 hover:shadow-[0_0_24px_rgba(0,150,137,0.3)]"
        >
    ...
    <section className="rounded-2xl border border-white/10 bg-zinc-900/65 p-5 backdrop-blur-xl">
      ...
      <div className="space-y-3">
        {recentActivity.map((entry) => (
          <div
            key={entry.id}
            className="rounded-xl border border-white/10 bg-white/[0.03] px-4 py-3 text-sm text-zinc-200 transition hover:border-[#009689]/40 hover:bg-[#009689]/10"
          >
            {entry.action}
          </div>
        ))}
