  ...
  return (
    <section className="space-y-5 rounded-2xl border border-white/10 bg-zinc-900/65 p-5 backdrop-blur-xl">
        ...
        {testimonials.map((item) => (
          <div key={item.id} className="rounded-xl border border-white/10 bg-black/20 p-4 text-sm text-zinc-200">
            ...
            <div className="mt-3 flex gap-2">
              <button onClick={() => { setEditingId(item.id); setForm({ name: item.name, role: item.role, message: item.message }); }} className="rounded border border-white/10 px-3 py-1 hover:border-[#155dfc]/50">Edit</button>
              <button onClick={() => setTestimonials((prev) => prev.filter((entry) => entry.id !== item.id))} className="rounded border border-white/10 px-3 py-1 hover:border-[#155dfc]/50 hover:text-[#155dfc]">Delete</button>
            </div>
          </div>
        ))}
      </div>
      ...
      <form onSubmit={saveTestimonial} className="space-y-3 rounded-xl border border-white/10 bg-black/20 p-4">
        ...
        <button type="submit" className="rounded-xl border border-[#155dfc]/60 bg-[#155dfc]/15 px-4 py-2 text-sm text-[#155dfc]">Save Testimonial</button>
      </form>
    </section>
  );
};
