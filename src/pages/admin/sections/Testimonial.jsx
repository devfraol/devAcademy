  ...
  return (
    <section className="space-y-5 rounded-2xl border border-white/10 bg-zinc-900/65 p-5 backdrop-blur-xl">
        ...
        {testimonials.map((item) => (
          <div key={item.id} className="rounded-xl border border-white/10 bg-black/20 p-4 text-sm text-zinc-200">
            ...
            <div className="mt-3 flex gap-2">
              <button onClick={() => { setEditingId(item.id); setForm({ name: item.name, role: item.role, message: item.message }); }} className="rounded border border-white/10 px-3 py-1 hover:border-[#009689]/50">Edit</button>
              <button onClick={() => setTestimonials((prev) => prev.filter((entry) => entry.id !== item.id))} className="rounded border border-white/10 px-3 py-1 hover:border-[#009689]/50 hover:text-[#009689]">Delete</button>
            </div>
          </div>
        ))}
      </div>
      ...
      <form onSubmit={saveTestimonial} className="space-y-3 rounded-xl border border-white/10 bg-black/20 p-4">
        ...
        <button type="submit" className="rounded-xl border border-[#009689]/60 bg-[#009689]/15 px-4 py-2 text-sm text-[#009689]">Save Testimonial</button>
      </form>
    </section>
  );
};
