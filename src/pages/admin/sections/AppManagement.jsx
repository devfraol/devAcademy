...
export const AppManagement = () => {
  ...
  return (
    <section className="rounded-2xl border border-white/10 bg-zinc-900/65 p-5 backdrop-blur-xl">
      <div className="mb-4 flex flex-wrap items-center justify-between gap-3"><h2 className="text-lg font-semibold text-white">App Management</h2><button onClick={() => setIsOpen(true)} className="rounded-xl border border-[#007a55]/60 bg-[#007a55]/15 px-4 py-2 text-sm font-medium text-[#FF7C73] transition hover:bg-[#007a55]/25 hover:text-white">Add New App</button></div>
      ...
      {isOpen && <div className="fixed inset-0 z-50 grid place-items-center bg-black/70 p-4"><form onSubmit={addApp} className="w-full max-w-lg space-y-4 rounded-2xl border border-white/10 bg-zinc-950 p-5"><h3 className="text-lg font-semibold text-white">Add New App</h3><input value={form.name} onChange={(event) => setForm((prev) => ({ ...prev, name: event.target.value }))} placeholder="App Name" className="w-full rounded-xl border border-white/10 bg-zinc-900 px-3 py-2 text-white outline-none focus:border-[#007a55]/65" /><textarea value={form.description} onChange={(event) => setForm((prev) => ({ ...prev, description: event.target.value }))} placeholder="Description" rows={3} className="w-full rounded-xl border border-white/10 bg-zinc-900 px-3 py-2 text-white outline-none focus:border-[#007a55]/65" /><div className="flex justify-end gap-2"><button type="button" onClick={() => setIsOpen(false)} className="rounded-xl border border-white/10 px-4 py-2 text-zinc-300">Cancel</button><button type="submit" disabled={loading.submit} className="rounded-xl border border-[#007a55]/60 bg-[#007a55]/15 px-4 py-2 text-[#FF7C73]">{loading.submit ? "Saving..." : "Save App"}</button></div></form></div>}
    </section>
  );
};
