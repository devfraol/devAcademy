    ...
    return (
      <section className="mx-auto flex min-h-[60vh] w-full max-w-[1240px] items-center justify-center px-4 py-16 sm:px-6">
        <div className="rounded-3xl border border-[#232326] bg-[#151518] p-8 text-center">
          ...
          <Link to="/courses" className="mt-4 inline-flex rounded-xl border border-[#007a55] px-4 py-2 text-sm font-medium text-[#007a55]">
            Back to courses
          </Link>
        </div>
      </section>
        ...
        <div className="space-y-8">
          <header className="rounded-[24px] border border-[#232326] bg-[#151518] p-6 sm:p-8">
            <div className="grid items-center gap-8 lg:grid-cols-[minmax(0,1fr)_300px]">
              <div>
                <span className="inline-flex rounded-full bg-[#007a55]/20 px-3 py-1 text-sm font-medium text-[#58b699]">{course.category}</span>
                ...
                <div className="mt-5 flex flex-wrap items-center gap-4 text-sm text-[#A1A1AA]">
                  ...
                  <span className="inline-flex items-center gap-1 text-[#007a55]">
                ...
                <div className="mt-6 flex flex-wrap gap-3">
                  <motion.button
                    ...
                    className="rounded-xl bg-[#007a55] px-5 py-3 text-sm font-semibold text-white transition hover:shadow-[0_0_22px_rgba(225,6,0,0.35)]"
                  >
                    Enroll Now
                  </motion.button>
                  <motion.button
                    whileTap={{ scale: 0.97 }}
                    type="button"
                    className="rounded-xl border border-[#007a55] px-5 py-3 text-sm font-semibold text-[#007a55] transition hover:bg-[#007a55]/10"
                  >
                    Preview Syllabus
                  </motion.button>
                </div>
            ...
            <div className="mt-5 flex flex-col gap-6 lg:flex-row lg:items-center lg:justify-between">
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="mt-1 inline-flex items-center gap-1 text-sm text-[#007a55]"><Star className="h-4 w-4 fill-current" /> {course.instructor.rating ?? 4.9}</p>
                </div>
              </div>

              <div className="max-w-xl">
                <p className="line-clamp-4 text-sm leading-7 text-[#A1A1AA]">{course.instructor.bio}</p>
                <button type="button" className="mt-4 rounded-xl border border-[#007a55] px-4 py-2 text-sm font-medium text-[#007a55] transition hover:bg-[#007a55]/10">
                  View Profile
                </button>
              </div>
            </div>
              ...
              {learningPoints.map((point) => (
                <article key={point} className="rounded-xl border border-[#232326] bg-[#151518] p-4">
                  <p className="flex items-start gap-3 text-sm text-[#A1A1AA]">
                    <Check className="mt-0.5 h-4 w-4 shrink-0 text-[#007a55]" />
                    <span>{point}</span>
                  </p>
                </article>
              ))}
        ...
        <aside className="hidden lg:block lg:sticky lg:top-24">
          <div className="rounded-2xl border border-[#232326] bg-[#151518] p-6">
            ...
            <motion.button
              ...
              type="button"
              onClick={() => navigate(`/courses/${course.slug}/learn`)}
              className="mt-5 w-full rounded-xl bg-[#007a55] px-4 py-3 text-sm font-semibold text-white transition hover:shadow-[0_0_24px_rgba(225,6,0,0.35)]"
            >
              Enroll Now
            </motion.button>
            <button type="button" className="mt-3 w-full rounded-xl border border-[#007a55] px-4 py-3 text-sm font-semibold text-[#007a55] transition hover:bg-[#007a55]/10">
              Add to Wishlist
            </button>

            <ul className="mt-6 space-y-3 text-sm text-[#A1A1AA]">
              <li className="flex items-center gap-2"><BookOpen className="h-4 w-4 text-[#007a55]" /> Lifetime access</li>
              <li className="flex items-center gap-2"><FileText className="h-4 w-4 text-[#007a55]" /> Text-based lesson notes</li>
              <li className="flex items-center gap-2"><Clock3 className="h-4 w-4 text-[#007a55]" /> {formatDuration(course.totalMinutes)} self-paced content</li>
            </ul>
      ...
      <div className="fixed inset-x-0 bottom-0 z-50 border-t border-[#232326] bg-[#151518] p-3 lg:hidden">
        <div className="mx-auto flex w-full max-w-[1240px] items-center gap-3">
          ...
          <motion.button
            ...
            type="button"
            onClick={() => navigate(`/courses/${course.slug}/learn`)}
            className="rounded-xl bg-[#007a55] px-4 py-3 text-sm font-semibold text-white"
          >
            Enroll Now
          </motion.button>
        </div>
