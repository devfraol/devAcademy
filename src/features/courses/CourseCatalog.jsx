...
const VISIBLE_CATEGORY = "Operate Computer";
...
export const CourseCatalog = () => {
  ...
  const [activeCategory, setActiveCategory] = useState(VISIBLE_CATEGORY);
  ...
  useEffect(() => {
    const loadCourses = async () => {
      try {
        ...
        const filteredNormalized = normalized.filter((course) => course.category === VISIBLE_CATEGORY);

        if (!filteredNormalized.length) {
          throw new Error("No API courses available for Dev Fraol");
        }

        setCourses(filteredNormalized);
      } catch {
        const localCourses = fallbackCourses.map(normalizeFallbackCourse).filter((course) => course.category === VISIBLE_CATEGORY);
