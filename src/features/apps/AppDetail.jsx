    ...
    <main className="px-4 py-12 sm:px-6 lg:px-8">
      <div className="mx-auto flex w-full max-w-6xl flex-col gap-8 text-left">
        <motion.section initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} className="glass-panel rounded-3xl p-7 sm:p-10">
          <div className="inline-flex rounded-2xl bg-[#009689]/15 p-4 text-[#009689]">
          ...
          <div className="mt-8 flex flex-wrap gap-3">
            <button onClick={handleOpenApp} type="button" className="inline-flex items-center gap-2 rounded-xl border border-[#009689]/70 bg-[#009689]/85 px-5 py-3 text-sm font-semibold text-white shadow-[0_12px_30px_rgba(0,150,137,0.35)] backdrop-blur-xl transition hover:scale-[1.03] hover:shadow-[0_16px_35px_rgba(0,150,137,0.45)]">
              {actionLoading[`open-app:${appId}`] ? "Opening..." : "Open App"}
              <ArrowUpRight className="h-4 w-4" />
            </button>
            <Link to="/apps" className="inline-flex items-center gap-2 rounded-xl border border-white/20 bg-white/10 px-5 py-3 text-sm font-semibold text-foreground backdrop-blur-xl transition hover:scale-[1.02] hover:border-[#009689]/60 hover:text-[#009689]">
            ...
            <button
              ...
              className="inline-flex items-center rounded-xl border border-white/20 bg-black/30 px-5 py-3 text-sm font-semibold text-foreground/90 transition hover:border-[#009689]/60 hover:text-[#009689]"
            >
              Explore Details
            </button>
          </div>
          ...
          <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, amount: 0.2 }} className="glass-panel rounded-3xl p-7 sm:p-9">
            ...
            {demoUrl && (
              <div className="mt-8 rounded-2xl border border-white/10 bg-black/25 p-5">
                ...
                <a href={demoUrl} target="_blank" rel="noreferrer" className="mt-3 inline-flex items-center gap-2 rounded-xl bg-[#009689] px-4 py-2 text-sm font-semibold text-white transition hover:shadow-[0_10px_25px_rgba(0,150,137,0.35)]">
                  Watch Demo
                  <ArrowUpRight className="h-4 w-4" />
                </a>
              </div>
          ...
          <section className="glass-panel rounded-3xl p-7 sm:p-9">
            ...
            <div className="mt-6 flex flex-col gap-4 rounded-2xl border border-white/10 bg-black/30 p-4 sm:flex-row sm:items-center sm:justify-between">
              <label className="relative w-full sm:max-w-sm">
                ...
                <input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search websites" className="w-full rounded-xl border border-white/10 bg-black/25 py-2.5 pl-9 pr-3 text-sm outline-none focus:border-[#009689]/65" />
              </label>
              <div className="flex flex-wrap gap-2">
                {categories.map((item) => (
                  <motion.button key={item} type="button" whileTap={{ scale: 0.96 }} onClick={() => setCategory(item)} className={`rounded-full px-4 py-2 text-sm font-semibold transition ${category === item ? "bg-[#009689] text-white shadow-[0_10px_20px_rgba(0,150,137,0.28)]" : "border border-white/10 text-foreground/80 hover:border-[#009689]/50 hover:text-[#009689]"}`}>
                    {item}
                  </motion.button>
                ))}
              </div>
        ...
        <section className="glass-panel rounded-3xl p-7 sm:p-9">
          ...
          <div className="mt-4 flex flex-wrap gap-3">
            ...
            {resources.map((resource) => {
              ...
              if (isExternal) {
                return (
                  <a key={resource.name} href={resource.link} target="_blank" rel="noreferrer" className="rounded-xl border border-white/10 bg-black/30 px-4 py-2 text-sm font-semibold text-foreground transition hover:border-[#009689]/60 hover:text-[#009689]">
                    {resource.name}
                  </a>
                );
              }
              ...
              return (
                <Link key={resource.name} to={resource.link} className="rounded-xl border border-white/10 bg-black/30 px-4 py-2 text-sm font-semibold text-foreground transition hover:border-[#009689]/60 hover:text-[#009689]">
                  {resource.name}
                </Link>
              );
            })}
        ...
        <footer className="pb-2">
          <Link to="/apps" className="inline-flex items-center gap-2 rounded-xl border border-[#009689]/55 bg-[#009689]/15 px-5 py-3 text-sm font-semibold text-[#009689] transition hover:scale-[1.02]">
            Return to Apps Page
            <ArrowLeft className="h-4 w-4" />
          </Link>
        </footer>
