      ...
      <Section>
        <div className="mb-6 flex flex-col gap-4 glass-panel rounded-2xl p-4 sm:flex-row sm:items-center sm:justify-between">
          <label className="relative w-full sm:max-w-sm" htmlFor="app-search">
            ...
            <input id="app-search" value={appQuery} onChange={(e) => setAppQuery(e.target.value)} placeholder="Search apps" className="w-full rounded-xl border border-white/10 bg-black/25 py-2.5 pl-9 pr-3 text-sm outline-none focus:border-[#009689]/65" />
          </label>
          ...
          {!loading.list && filteredApps.map((app, index) => {
            ...
            return (
              <motion.div key={app.id} initial={{ opacity: 0, y: 22 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: index * 0.06 }}>
                <Card>
                  <div className="inline-flex rounded-xl bg-[#009689]/15 p-3 text-[#009689]">
      ...
      <Section className="mt-6">
        ...
        <div className="mt-6 mb-6 flex flex-col gap-4 glass-panel rounded-2xl p-4 sm:flex-row sm:items-center sm:justify-between">
          <label className="relative w-full sm:max-w-sm" htmlFor="website-search">
            <Search className="pointer-events-none absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-foreground/45" />
            <input id="website-search" value={webQuery} onChange={(e) => setWebQuery(e.target.value)} placeholder="Search websites" className="w-full rounded-xl border border-white/10 bg-black/25 py-2.5 pl-9 pr-3 text-sm outline-none focus:border-[#009689]/65" />
          </label>
