  ...
  return (
    ...
    >
      ...
      <ul className="mt-5 space-y-4">
        {contactDetails.map(({ label, value, href }) => {
          ...
          <li key={label} className="flex items-start gap-3">
            <span className="mt-0.5 rounded-lg border border-white/10 bg-black/30 p-2 text-[#007a55]">
              <Icon className="h-4 w-4" aria-hidden="true" />
            </span>
            <div>
              <p className="text-sm text-zinc-400">{label}</p>
              {href ? (
                <a href={href} className="text-zinc-100 transition hover:text-[#007a55]">
                  {value}
                </a>
              ) : (
      ...
      <div className="mt-6 border-t border-white/10 pt-5">
        ...
        <SocialLinks className="gap-2" iconClassName="bg-black/30 border-white/10 text-zinc-200 hover:text-[#007a55]" iconOnly />
      </div>
    </motion.article>
  );
};
