...
export const TeachingPhilosophy = () => {
  ...
  return (
    <section aria-labelledby="teaching-philosophy" className="relative mt-14 overflow-hidden rounded-3xl border border-border/60 bg-card/45 p-6 text-left sm:p-10">
      <motion.div
        className="pointer-events-none absolute -right-20 -top-20 h-52 w-52 rounded-full bg-[#155dfc]/20 blur-3xl"
        ...
        {tabList.map((tab) => {
          ...
          return (
            <button
              ...
              className={`rounded-full border px-5 py-2 text-sm font-semibold transition focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#155dfc] focus-visible:ring-offset-2 focus-visible:ring-offset-background ${
                selected
                  ? "border-[#155dfc]/80 bg-[#155dfc]/15 text-[#155dfc]"
                  : "border-border/80 bg-background/40 text-muted-foreground hover:border-[#155dfc]/50 hover:text-[#155dfc]"
              }`}
            >
              {tab}
            </button>
      ...
      <div className="relative mt-7 min-h-40 rounded-2xl border border-border/70 bg-background/55 p-6">
        <AnimatePresence mode="wait">
          ...
          >
            <h3 className="text-xl font-semibold text-[#155dfc]">{current.title}</h3>
