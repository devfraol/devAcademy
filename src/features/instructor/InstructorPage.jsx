    ...
    <section className="px-4 pb-16 sm:px-6" aria-label="Instructor page">
      <div className="container mx-auto max-w-6xl space-y-12 text-left">
        <header className="relative overflow-hidden rounded-3xl border border-border/60 bg-card/55 px-6 py-14 sm:px-10">
          <p className="inline-flex rounded-full border border-[#009689]/40 bg-[#009689]/12 px-4 py-1 text-sm font-semibold tracking-wide text-[#009689]">Instructor Spotlight</p>
          <h1 className="mt-5 text-4xl font-black sm:text-5xl md:text-6xl">{instructor?.name}</h1>
          <p className="mt-2 text-[#009689]">{instructor?.role}</p>
          ...
          <ul className="mt-6 space-y-3 text-muted-foreground">
            {(instructor?.experience ?? []).map((point) => (
              <li key={point} className="flex items-start gap-3">
                <span className="mt-2 h-2 w-2 shrink-0 rounded-full bg-[#009689]" aria-hidden="true" />
                <span>{point}</span>
              </li>
            ))}
          </ul>
        ...
        <section className="rounded-3xl border border-border/60 bg-card/45 p-6 sm:p-10">
          ...
          <div className="mt-6 flex flex-wrap gap-3">
            {philosophyTabs.map((tab) => (
              <button key={tab} onClick={() => setActiveTab(tab)} className={`rounded-full border px-5 py-2 text-sm font-semibold ${activeTab === tab ? "border-[#009689]/80 bg-[#009689]/15 text-[#009689]" : "border-border/80 text-muted-foreground"}`}>{tab}</button>
            ))}
          </div>
          {currentTab ? (
            <div className="mt-7 min-h-40 rounded-2xl border border-border/70 bg-background/55 p-6">
              <AnimatePresence mode="wait">
                <motion.article key={activeTab} initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -12 }}>
                  <h3 className="text-xl font-semibold text-[#009689]">{currentTab.title}</h3>
        ...
        <section className="rounded-3xl border border-[#009689]/45 bg-linear-to-r from-[#009689]/20 via-card/70 to-[#009689]/10 px-6 py-12 text-center sm:px-10">
          ...
          <div className="mt-8 inline-flex">
            <Link to="/apps" className="inline-flex items-center justify-center rounded-xl bg-[#009689] px-8 py-3 text-base font-semibold text-white">View Apps</Link>
          </div>
        </section>
      </div>
    </section>
