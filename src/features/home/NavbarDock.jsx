...
const dockLinkClassName = ({ isActive }) =>
  cn(
    ...
    isActive
      ? "border-[#007a55]/70 bg-[#3A1D1A]/80 text-[#007a55] shadow-[0_0_22px_rgba(0,122,85,0.35)]"
      : "border-white/10 bg-white/5 text-[#D4D4D4] hover:border-[#007a55]/55 hover:text-[#007a55] hover:shadow-[0_0_20px_rgba(0,122,85,0.25)]"
  );
    ...
    <NavLink to={to} end={to === "/"} className={dockLinkClassName} aria-label={`Go to ${label}`}>
      {({ isActive }) => (
        <>
          ...
          {isActive && <motion.span layoutId="dock-active" className="absolute -bottom-1 left-1/2 h-1.5 w-1.5 -translate-x-1/2 rounded-full bg-[#007a55]" />}
        </>
      )}
    </NavLink>
  </motion.div>
