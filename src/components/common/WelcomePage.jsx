    ...
    <AnimatePresence>
      {!isExiting && (
        ...
        >
          ...
          <motion.div
            className="absolute inset-0 bg-[radial-gradient(circle_at_20%_20%,rgba(0,150,137,0.25),transparent_45%),radial-gradient(circle_at_80%_0%,rgba(0,150,137,0.16),transparent_35%),radial-gradient(circle_at_60%_80%,rgba(255,255,255,0.08),transparent_40%)]"
          ...
          {particles.map((particle) => (
            <motion.span
              key={particle.id}
              className={`absolute rounded-full bg-[#009689]/60 ${particle.size}`}
          ...
          <div className="relative w-full max-w-2xl rounded-3xl border border-white/10 bg-white/5 p-6 shadow-[0_0_90px_rgba(0,150,137,0.2)] backdrop-blur-xl sm:p-9">
            <motion.img
              src={logoDark}
              alt="Dev Fraol Academy"
              className="mx-auto w-52 sm:w-64 md:w-72"
              initial={{ opacity: 0, scale: 0.84, y: 18 }}
              animate={{ opacity: 1, scale: 1, y: 0, filter: "drop-shadow(0 0 18px rgba(0,150,137,0.45))" }}
            ...
            >
              <p className="text-lg font-semibold tracking-wide text-white sm:text-2xl">
                {typedMessage}
                <span className="ml-1 inline-block h-5 w-[2px] translate-y-1 animate-pulse bg-[#009689]" />
              </p>
